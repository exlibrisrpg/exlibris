...
class IconHelperTest < ActionView::TestCase
  context "#icon_tag" do
    setup do
      def self.inline_svg_pack_tag
      end
    end

    should "call inline_svg_pack_tag with the correct path" do
      svg_tag = "<svg></svg"
      expected_path = "media/svg/icon-name.svg"
      expected_options = {class: "icon"}

      stub :inline_svg_pack_tag, ->(path, options) { svg_tag if path == expected_path && options == expected_options } do
    ...
    should "call inline_svg_pack_tag with the correctly formatted path" do
      svg_tag = "<svg></svg"
      expected_path = "media/svg/icon-longer-name.svg"
      expected_options = {class: "icon"}
